...
public static class IdentityApiEndpointRouteBuilderExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
            ([FromBody] Gaolos.Identity.Data.RegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            await userManager.SetNameAsync(user, registration.Name);
            await userManager.SetSurnameAsync(user, registration.Surname);
        ...
        accountGroup.MapGet("/info", async Task<Results<Ok<Gaolos.Identity.Data.InfoResponse>, ValidationProblem, NotFound>>
    ...
    private static async Task<Gaolos.Identity.Data.InfoResponse> CreateInfoResponseAsync<TUser>(TUser user, ApplicationUserManager userManager)
        where TUser : ApplicationUser
    {

        return new Gaolos.Identity.Data.InfoResponse
        {
            UserId = await userManager.GetUserIdAsync(user) ?? throw new NotSupportedException("Users must have an ID."),
            Email = await userManager.GetEmailAsync(user) ?? throw new NotSupportedException("Users must have an email."),
            Name = await userManager.GetNameAsync(user) ?? throw new NotSupportedException("Users must have a name."),
            Surname = await userManager.GetSurnameAsync(user) ?? throw new NotSupportedException("Users must have a surname."),
